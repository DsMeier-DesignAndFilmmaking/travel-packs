...
/**
 * CityGuideView
 * Guide path view with cache-buster manifest URL to bypass cache deadlock.
 */
export function CityGuideView({ pack }: { pack: CityPack }) {
  const { slug } = useParams<{ slug: string }>();
  const { installPrompt, isInstalled, handleInstall } = usePWAInstall();
  const [showMobileOverlay, setShowMobileOverlay] = useState(false);

  useEffect(() => {
    // 1. Critical Guard: Prevents manifest request with undefined slug
    if (!slug || slug === 'undefined') return;

    // 2. Cache-buster: use API URL with ?v= to bypass SW/document cache deadlock
    ...
    // 5. Cleanup: reset head on unmount
    return () => {
      setHomeHead();
    };
  }, [slug, pack?.city]);
